...
allowed-tools: Bash(gh issue list:*), Bash(gh issue view:*), Bash(gh pr list:*), Bash(gh pr view:*), Bash(gh pr checks:*), Bash(gh api user:*), Bash(gh label list:*), Bash(gh run list:*)
---
...
3. **Fill dev then flex slots** from the issue queue: open issues sorted P0 → P1 → P2, excluding claimed, operator-held, and not-dispatchable labels (taxonomy in the issue control-field skill — gh-issue-labels for GitHub, jira-issue-fields for Jira — and the issue-filing skill — gh-issue-filing / jira-issue-filing). If this repo is a meta repo (has a `registry/repos.yaml`) and a candidate carries a `repo:<name>` label, resolve and provision it per the **registry-dispatch** skill before proceeding — an unresolvable `repo:<name>` is not dispatchable, same as any other exclusion label. Bundle same-surface small issues into one dispatch — review cost is per-PR, not per-issue. For each dispatch: lock per the issue-locking skill (gh-issue-locking / jira-issue-locking), resolve model/effort per the control-field skill, write the eight-section brief per dispatching-subagents (cwd/PR-target/Closes per registry-dispatch when a target repo was resolved), and launch independent dispatches in the same message so they run in parallel.
