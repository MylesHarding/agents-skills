...
- `.claude/agents/<agent-name>.md` — one file per agent; frontmatter (`name`, `description`, `tools`, optional `model`) + system prompt. Ships 15 tiered worker agents (`scout` on the cheap tier; `builder`, `reviewer`, `implementer`, `pr-rescuer`, `diagnostician`, `verifier`, `spelunker`, `design-mapper` on workhorse — these the commands dispatch — plus the `pr-comments`, `pr-checks`, `pr-cleanup` single-lane PR watchers and the `ci-speed-hunter`, `ci-flake-hunter` continuous CI lanes and the `branch-ff` fast-forward lane, all run via `/loop`). Canonical agents live here; `make sync-agents` generates the Cursor (`.cursor/agents/`, `model`/`readonly` schema), Copilot (`.github/agents/*.agent.md`), Kiro (`.kiro/agents/`, `tools` category allowlist) and OpenCode (`.opencode/agents/`, `mode`/`permission`) copies — never hand-edit those. Keep agents thin and point at the relevant skill for the protocol.
- `.claude-plugin/marketplace.json` + `plugins/agents-skills/` (Claude Code) and `.cursor-plugin/marketplace.json` + `plugins/agents-skills-cursor/` (Cursor) — this repo doubles as its own plugin marketplace for **both** ecosystems. Same skills in each (the Agent Skills format is identical); agents and commands come from that tool's generated mirror, since their frontmatter differs. The plugin payload is **generated** from `.claude/{skills,agents,commands}` by `make build-plugin` (real copies, not symlinks — Claude Code installs the plugin dir standalone, so a link escaping it would dangle); `make check-plugin` fails on drift. Never hand-edit `plugins/`. Releases are cut by `.github/workflows/release-plugin.yml`: CalVer `YYYY.M.D.N` (patch counter so same-day releases never collide), tag `v<version>`, assets = a full-plugin zip + one zip per skill (the latter for Claude Desktop). The workflow stamps versions — never bump them by hand.
