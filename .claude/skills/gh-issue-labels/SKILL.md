...
| `blocked` | Issues | Technical recon (`<blocked-state>` in the technical-recon skill) | Orchestrator, when scanning for stalled pipeline state | A dependency issue/PR/infra must land first. Not terminal — recon revisits once the lead adjusts direction or the dependency clears. Bootstrap this label: an orchestration loop that force-normalizes any issue with no recognized pipeline-state label back to a "needs grooming" state will otherwise treat a legitimately-blocked issue as orphaned and re-queue it forever |
